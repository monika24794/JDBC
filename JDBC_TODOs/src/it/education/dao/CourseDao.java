...
public class CourseDao  implements DaoInterface<Course, Integer>{
	...
	public Collection<Course> retriveAll() {
				...
				try (Connection dbConnection = JdbcUtils.buildConnection();
					...
					while (rs.next()) {
						...
						Course currentcorse = 
						new Course(courseId, name, provider, duration, fees);
	...
	public Course retrieveOne(Integer id) {
		...
		String sqlQuery = 
				"select course_id,course_name,course_provider,course_duration, course_fees from course_master where course_id=? ";
