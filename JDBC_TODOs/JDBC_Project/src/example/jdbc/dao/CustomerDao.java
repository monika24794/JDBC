...
public class CustomerDao implements DaoInterface<Customer, Integer> {
	...
	@Override
	public void update(Customer modifieadCustomer) {
		// we can update both or any one
		String sqlQuery = "UPDATE customer1 set cname=?, address=? where custid=?";
		int id=modifieadCustomer.getCustomerId();
		String newname=modifieadCustomer.getName();
		String newaddress=modifieadCustomer.getAddress();
